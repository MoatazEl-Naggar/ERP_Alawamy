...
export default function Purchases() {
  ...
  // ── Add item row ─────────────────────────────────────────────────────────────
  const commitItem = () => {
    if (!draftItem.itemCode && !draftItem.itemName) { toast(t("itemCodeRequired"), "warning"); return; }
  ...
  setForm({
    ...
    // Map DB fields → frontend PurchaseItem shape
    items: (inv.items ?? []).map((it: any) => ({
      barcode:            it.barcode        ?? "",
      itemCode:           it.itemCode       ?? it.itemName ?? "",
      qtyCartons:         it.qtyCartons     ?? 0,
      qtyUnits:           it.qtyUnits       ?? 0,
      totalUnits:         (it.qtyCartons ?? 0) * (it.qtyUnits ?? 0),
      category:           it.category       ?? "",
      price:              it.price          ?? 0,
      value:              (it.qtyCartons ?? 0) * (it.qtyUnits ?? 0) * (it.price ?? 0),
      itemDiscount:       it.discount       ?? 0,
      valueAfterDiscount: it.total          ?? 0,
      description:        it.description    ?? "",
      cartonNumber:       it.cartonNumber   ?? "",
      receivingDate:      it.receivingDate
                            ? new Date(it.receivingDate).toISOString().split("T")[0]
                            : "",
      cbm:                it.cbm            ?? 0,
  ...
  const handlePrint = () => {
    ...
    const rows = form.items.map((it, i) => `
      <tr>
        <td>${i + 1}</td><td>${it.itemCode}</td><td>${it.itemName || "—"}</td><td>${it.barcode || "—"}</td>
    ...
    w.document.write(`<html><head><title>${t("purchasesTitle")}</title>
      <style>
        body{font-family:Arial;direction:rtl;padding:20px;font-size:11px}
        ...
        .total{font-weight:bold;margin-top:10px;font-size:13px;text-align:left}
      </style></head><body>
      ...
      <div class="header">
        ...
        <span><b>${t("supplier")}:</b> ${supplierName}</span>
      ...
      <table>
        <thead><tr>
          <th>#</th><th>${t("itemCode")}</th><th>${t("itemName")}</th><th>${t("barcode")}</th>
      ...
      <div class="total">${t("grandTotal")}: ${(grandTotal - (form.headerDiscount || 0)).toLocaleString()}</div>
    ...
    <Box sx={{ p: { xs: 1, md: 2 } }}>
          ...
          <Paper elevation={2} sx={{ p: 2, borderTop: `4px solid ${BLUE}`, borderRadius: 2 }}>
            ...
            <Box sx={{ bgcolor: BLUE_LIGHT, borderRadius: 1, p: 1.5, mb: 2 }}>
              ...
              <Grid container spacing={1.5}>
                ...
                {/* Container No — dropdown from containers list */}
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth size="small" select label={t("containerNo")}
                    ...
                    {containers.map(c => (
                      <option key={c.id} value={c.containerNumber}>{c.containerNumber}</option>
                    ))}
                  </TextField>
                </Grid>
                ...
                {/* Client Code — dropdown from customers */}
                <Grid item xs={6} sm={3}>
            ...
            <Box sx={{ bgcolor: "#FAFAFA", border: "1px solid #E0E0E0", borderRadius: 1, p: 1.5, mb: 1.5 }}>
              ...
              <Grid container spacing={1} alignItems="flex-end">
                ...
                {/* Item Name */}
                <Grid item xs={6} sm={2}>
                  <TextField fullWidth size="small" label={t("itemName")}
                    value={draftItem.itemName} onChange={e => patchItem({ itemName: e.target.value })} />
                </Grid>

                {/* Item Code */}
                <Grid item xs={6} sm={2}>
                  <TextField fullWidth size="small" label={t("itemCode")}
                    value={draftItem.itemCode} onChange={e => patchItem({ itemCode: e.target.value })} />
                </Grid>
                
                {/* Cartons */}
                <Grid item xs={4} sm={1}>
                  <TextField fullWidth size="small" label={t("cartons")} type="number"
                    value={draftItem.qtyCartons} onChange={e => patchItem({ qtyCartons: +e.target.value })} />
                </Grid>

                {/* Units */}
                <Grid item xs={4} sm={1}>
                  <TextField fullWidth size="small" label={t("units")} type="number"
                    value={draftItem.qtyUnits} onChange={e => patchItem({ qtyUnits: +e.target.value })} />
                </Grid>
                ...
                <Grid item xs={4} sm={1.5}>
                  <TextField fullWidth size="small" label={t("category")}
                    value={draftItem.category} onChange={e => patchItem({ category: e.target.value })} />
                </Grid>

                {/* Price */}
                <Grid item xs={4} sm={1}>
                  <TextField fullWidth size="small" label={t("price")} type="number"
                    value={draftItem.price} onChange={e => patchItem({ price: +e.target.value })} />
                </Grid>
                ...
                {/* Item Discount */}
                <Grid item xs={4} sm={1}>
                  <TextField fullWidth size="small" label={t("itemDiscount")} type="number"
                    value={draftItem.itemDiscount} onChange={e => patchItem({ itemDiscount: +e.target.value })} />
                </Grid>
                ...
                <Grid item xs={12} sm={3}>
                  <TextField fullWidth size="small" label={t("description")}
                    value={draftItem.description} onChange={e => patchItem({ description: e.target.value })} />
                </Grid>

                {/* Carton Number */}
                <Grid item xs={6} sm={2}>
                  <TextField fullWidth size="small" label={t("cartonNumber")}
                    value={draftItem.cartonNumber} onChange={e => patchItem({ cartonNumber: e.target.value })} />
                </Grid>

                {/* Receiving Date */}
                <Grid item xs={6} sm={2}>
                  <TextField fullWidth size="small" label={t("receivingDate")} type="date"
                    value={draftItem.receivingDate} InputLabelProps={{ shrink: true }}
                    onChange={e => patchItem({ receivingDate: e.target.value })} />
                </Grid>

                {/* CBM */}
                <Grid item xs={4} sm={1}>
                  <TextField fullWidth size="small" label={t("cbm")} type="number"
                    value={draftItem.cbm} onChange={e => patchItem({ cbm: +e.target.value })} />
                </Grid>

                {/* Item Notes */}
                <Grid item xs={8} sm={3}>
                  <TextField fullWidth size="small" label={t("itemNotes")}
                    value={draftItem.itemNotes} onChange={e => patchItem({ itemNotes: e.target.value })} />
                </Grid>
            ...
            <Box sx={{ overflowX: "auto", mb: 1.5 }}>
              <Table size="small" sx={{ minWidth: 900 }}>
                ...
                <TableBody>
                  {form.items.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={17} align="center" sx={{ py: 4, color: "text.disabled", fontStyle: "italic" }}>
                        {t("noItemsYet")}
                      </TableCell>
                    </TableRow>
      ...
      <Dialog open={!!lightboxImg} onClose={() => setLightboxImg(null)} maxWidth="md">
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          {t("viewImage")}
          <IconButton onClick={() => setLightboxImg(null)}><ClearIcon /></IconButton>
        </DialogTitle>
        <DialogContent>
          {lightboxImg && <img src={lightboxImg} alt="" style={{ width: "100%", borderRadius: 8 }} />}
        </DialogContent>
      </Dialog>
    </Box>
  );
